...
penpot.ui.onMessage<string>(async (message) => {
  if (message === "create-comment") {
    ...
    const page = penpot.getPage();
    const comments = await page.findCommentThreads({onlyYours: true, showResolved: false})
    ...
    const page = penpot.getPage();
    const commentThreads = await page.findCommentThreads({onlyYours: true, showResolved: false});
